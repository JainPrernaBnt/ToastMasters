    ...
    fun fromDocument(document: DocumentSnapshot): DomainUser? {
        return try {
            ...
            val user = User(
                ...
                mentorNames = (data["mentorNames"] as? List<*>)?.filterIsInstance<String>() ?: emptyList(),
                ...
                roles = when {
                    ...
                    data["roles"] is List<*> -> (data["roles"] as List<*>).mapNotNull { 
                        try { UserRole.valueOf(it.toString()) } catch (e: Exception) { null }
                    }
                    else -> emptyList()
                },
                status = when {
                    ...
                    data["status"] != null -> {
                        try { User.Status.valueOf((data["status"] as String)) } 
                        catch (e: Exception) { User.Status.PENDING_APPROVAL }
                    }
            ...
            val role = user.roles.firstOrNull() ?: (data["role"]?.let { 
                try { UserRole.valueOf(it.toString()) } catch (e: Exception) { UserRole.MEMBER }
            } ?: UserRole.MEMBER)
